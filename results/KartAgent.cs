...
namespace KartGame.AI
{
    ...
    public class KartAgent : Agent, IInput
    {
        ...
        private void SetBehaviorParameters()
        {
            ...
            behaviorParameters.BrainParameters.NumStackedVectorObservations = 10;
        ...
        private void DrawDetectingRays()
        {
            ...
            DrawRays(Sensors[0], 7);
            DrawRays(Sensors[0], -7);
        }
        private void DrawRays(Sensor sensor)
        {
            var current = sensor;
            ...
            var endPoint = AgentSensorTransform.position + xform.forward * current.RayDistance;
        ...
        private void DrawRays(Sensor sensor, float angle)
        {
            var current = sensor;
            var xform = current.Transform;
            var hit = Physics.Raycast(AgentSensorTransform.position, (Quaternion.AngleAxis(angle, Vector3.forward) * xform.forward).normalized, out var hitInfo, current.RayDistance, Mask, QueryTriggerInteraction.Ignore);
            var endPoint = AgentSensorTransform.position + (Quaternion.AngleAxis(angle, Vector3.forward) * xform.forward).normalized * current.RayDistance;
            bool drawKartRadius = true;
            Color color = Color.blue;
            if (hit)
            {
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            if (triggered > 0)
            {
                ...
                else if (index != m_LastHitCheckpointIndex)
                {
                    Debug.Log("[Quick Stop Training Episode Ended] WrongCheckpoint ");
                    m_EndEpisode = true;
        ...
        public override void CollectObservations(VectorSensor sensor)
        {
            ...
            sensor.AddObservation(nextCollider.transform.position - m_Kart.transform.position);
            ...
            for (int i = 0; i < Sensors.Length; i++)
            {
                ...
                var hit1 = Physics.Raycast(AgentSensorTransform.position, Quaternion.AngleAxis(7, Vector3.forward) * xform.forward, out var hitInfo1,
                    current.RayDistance, Mask, QueryTriggerInteraction.Ignore);
                var hit2 = Physics.Raycast(AgentSensorTransform.position, Quaternion.AngleAxis(-7, Vector3.forward) * xform.forward, out var hitInfo2,
                    current.RayDistance, Mask, QueryTriggerInteraction.Ignore);
